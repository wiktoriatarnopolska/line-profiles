...
function plot_all(data)
    ...
    p = plot(title = _format_metric(data.metric) * incl_text, 
    xlims = (Gradus.isco(data.metric), 100), 
    minorgrid = true,
    xlabel = L"\textrm{r}~(rg)", ylabel = L"\textrm{Flux~(arbitrary~units)}",
    xscale = :log10, yscale = :log10,
    ylims = (1e-5, 1e4)
    )
    plot!([Gradus.isco(data.metric)], seriestype = "vline", label = L"\textrm{r}_{isco}", linecolor =:maroon, linewidth = 3)
    line_colors = [:darkorange3, :mediumvioletred, :purple3, :blue3]
    for (i, (edd, f)) in enumerate(zip((0, 10, 20, 30), data.f))
        radii = copy(f.radii)
        ems = copy(f.ε)
        if i >= 3
            push!(radii, radii[end] + (radii[end] - radii[end-1]))
            push!(ems, 1e-13)
        end
        plot!(p, radii, ems, label = _format_label(edd), linecolor = line_colors[i])
    end
    p
end
...
plot(pn3, pk3, pp3, pn2, pk2, pp2, pn1, pk1, pp1,layout = grid(3, 3), size = (1100, 1100), legend =:topright)
